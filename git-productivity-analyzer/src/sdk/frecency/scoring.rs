...
impl Analyzer {
    ...
    pub(super) fn age_weight_impl(&self, commit: &gix::Commit<'_>, now: i64) -> Result<f64> {
    ...
    pub(super) fn path_allowed_impl(&self, path: &PathBuf) -> bool {
        self.opts.paths.as_ref().map_or(true, |paths| paths.contains(path))
    }

    /// Update the score for a single file path.
    pub(super) fn update_score_impl(&self, scores: &mut HashMap<PathBuf, f64>, path: PathBuf, size: u64, weight: f64) {
        let penalty = self.size_penalty_impl(size);
        *scores.entry(path).or_default() += penalty * weight;
    }

    /// Return the penalty for a blob of the given size.
    pub(super) fn size_penalty_impl(&self, size: u64) -> f64 {
